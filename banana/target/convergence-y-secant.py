...
plt.rc('font', size=34)
plt.title(f'Convergence of the secant method')
...
for _ in range(128):
    ...
    ymax = np.tan(angle * np.pi/180) * xstar
    a, b = 30,32
...
#           break
    ...
    if len(err)<100:
        ...
        plt.plot(range(1,len(err)+1), err,c='red',alpha=.1)


if bananas_max<100:
    plt.text(8, 20, f'max throws to hit the target: {bananas_max}', dict(size=30))
else:
    plt.text(8, 20, f'max throws to hit the target: $\\infty$', dict(size=30))
...
plt.savefig("convergence-y-secant.png")
plt.show()
