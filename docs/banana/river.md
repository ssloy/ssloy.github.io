# Bateau en papier sur un ruisseau
...
On suppose connue une fonction `current(x,y)` qui, à tout point $(x,y)$ du plan, associe le vecteur vitesse du courant en ce point (celui-ci étant nul en dehors du ruisseau).
...
1. Modéliser mathématiquement la trajectoire du petit bateau à l’aide de deux équations différentielles couplées dépendant de la fonction `current(x,y)`.
...
et afficher à l’écran $(A_x, A_y)$ ainsi que le temps de trajet du bateau en papier entre Arthur et Basile.
